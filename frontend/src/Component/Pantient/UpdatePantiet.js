...
class UpdatePantient extends Component {
  
  constructor(props) {
    super(props)
    this.state = {
      URLID : props.match.params.idpantients,
      edad: 18,
      GetPantient: {}
    }
  }
  ...
  componentDidMount() {
    fetch(`http://localhost:4000/getonly/${this.state.URLID}`)
    .then(res => res.json())
    .then(data => this.setState({GetPantient: data.data[0]}))
    .catch(err => console.log(err))
  }

  render(){
    return(
      <Update pantient={this.state.GetPantient}/>
    )
  }
}

export default UpdatePantient

const Update = ({pantient}) => {
    ...
    return edad;
	 ...
	 const handleSubmit = ()=> {
		...
		const steps = [
			{ name: '1', component: <Step1 pantient={pantient} handlerStesp1={handlerStesp1} /> },
